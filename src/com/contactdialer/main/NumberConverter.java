...
class NumberConverter {
	...
	private void parse(NumberParserModel lt) {
		...
		if (matcher.find()) {
			...
			// /////
			country = UserModel.User.CHINA;
		}

		// if it is in China parse type
		if (country.equals(UserModel.User.CHINA)) {
	...
	@SuppressLint("DefaultLocale")
	private void parseFixed(NumberParserModel lt, String wcNumber,
			boolean hasCountry) {
		Pattern pattern;
		Matcher matcher;
		...
		for (int width = 1; width < 4; width++) {
			pattern = Pattern.compile(String.format("%s(\\d{%d})",
					districtHeader, width));
			matcher = pattern.matcher(wcNumber);
